...
if [[ $type == video/* ]]; then
  cache_dir="${XDG_CACHE_HOME:-$HOME/.cache}/fzf-video-thumbs"
  mkdir -p "$cache_dir"

  # Stable filename based on full path (handles duplicates)
  key=$(printf "%s" "$file" | sha1sum | awk "{print \$1}")
  img="$cache_dir/$key.jpg"

  # Create thumbnail if missing or video is newer than thumbnail
  if [ ! -s "$img" ] || [ "$file" -nt "$img" ]; then
    ffmpegthumbnailer -i "$file" -o "$img" -s 0 -q 5 2>/dev/null || {
  ...
  kitty icat --clear --transfer-mode=memory --stdin=no \
    --place=${FZF_PREVIEW_COLUMNS}x${FZF_PREVIEW_LINES}@0x0 "$img"
  exit
fi

kitty icat --clear --transfer-mode=memory
